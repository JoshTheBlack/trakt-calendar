...
### Changed
- ⚠️ **TV and film filters are now separate, and your existing genre and country filters stay with TV.** They were one answer applied to whichever calendar was open. If you had genres or countries set, your movie calendar will be wider the first time you open it after upgrading — nothing was lost, the film side simply starts unset, and each filter is one press to restate. Certifications were already separate, and the release filters were always films-only.
- 🎚️ **The Filters panel is two tabs instead of one long form**, and it opens on the calendar you are already looking at — 📺 TV on any show calendar, 🎬 Movies on the film one.
- 🎯 **Genres, countries, ratings and release formats are chips you press rather than lists you type.** Press once to keep only that one, again to filter it out, again to stop filtering on it — the same three answers, in the same two colors, that a card badge already offers. Anything the chips do not name is still one line of typing away, and the explanations that used to fill the panel are behind a small **i** beside each field.
- 📺 **Which services fill your calendar is asked once for TV and once for films.** One service has the deeper show coverage and the other the wider film listing, which is a preference no amount of narrowing could express. Each service is its own switch rather than a Both / one / the other picker, so a service added to this app later simply arrives switched on. At least one has to stay on.
- 🎛️ **The Filters button is now 🎚️ rather than 🔎**.

### Added
- 🆕 **A series premiere no longer looks like a fifth season.** On Season Premieres every card is somebody's episode 1, and telling a brand-new show from a returning one meant reading the season number off a small label. A first season now says **SERIES PREMIERE** and carries a gold frame; a later one says **RETURNING**. It marks episode 1 wherever episode 1 appears, so All Episodes flags the premieres inside it and leaves the rest of a season alone.
- 🎚️ **Every calendar now says how many cards your filters removed.** The count existed only on the film calendars, where a release filter can empty a month outright — but a genre or country exclude removes cards just as quietly, and a month that comes up short with nothing on the page to explain it reads as the app being broken. It counts only what you can undo: the instance-wide content floor is not yours to change and is not included.
- ⏸️ **Turn every filter off for a while without clearing any of them.** One switch at the top of the panel; your answers stay exactly where they are and come back the moment you turn it on again. The toolbar button says **Filters off** and looks different from both "filtered" and "nothing set", so a calendar showing everything never looks like an accident. Your share links keep filtering — switching your own view wide open does not quietly publish a wider calendar to everyone holding a link.

### Fixed
- 📺 **Two networks that differ only in capitals are two networks again.** `tvN` and `TVN` are a Korean broadcaster and a Polish one; adding both kept only the first, because the check for duplicates folded case while the matching does not.
- ⏸️ **Importing a month onto the tracker uses your filters even while they are switched off.** Turning filters off is a temporary look at a calendar and nothing is written by it — but an import writes rows, and those do not come back off when you turn the filters on again.
- ✅ **A not-watching mark that fails to save no longer looks saved.** The card was marked on screen before the answer came back and stayed marked if the save was refused, so a show could appear turned away and be imported onto the tracker anyway. A failed save now puts the card back.
- 📝 **The tracker no longer imports shows you have turned away on the calendar.** The import recognized only two of the ways a mark can be written down, where the calendar itself accepts every spelling a title has ever been known by — so a show marked under one service's name for it was hidden on the calendar and added to the month anyway. Both now ask the same question, in the same place. If a month already picked some up, remove those rows and re-import.
- 📱 **Filtering by a badge on a card now works on a phone.** Tapping a genre, country or network opened the title's details instead of the filter offer. A touch screen has no hover to enter, so a browser applies one on the first tap — moving the card under your finger mid-press. Hover effects meant for a mouse are now confined to pointers that can actually hover, so nothing shifts out from under a tap.

### Under the hood
- 🧮 **The panel is markup the server built.** Every chip, every tab and every answer arrives with the page, so opening the panel costs no request, switching tabs runs no code, and nothing in the browser holds a copy of the genre list or knows how a filter is written down. What is left is a press that cycles a chip and the save itself.
- 🗃️ **One place decides which stored filter answers for which kind of title**, so the calendar, a day loaded late, the search and a share link's page cannot come to different conclusions about it.
...
### Look and feel
- 🎨 **New artwork throughout.** A mark in the header, a proper wordmark on the sign-in and invite pages instead of the old squashed square, sharp favicons at both sizes, and a real 1200x630 preview card — so a shared link or an invite unfurls as a wide card rather than a thumbnail. The intermission page you land on at a dead end wears the wordmark too, drained of color to match the rest of that page.
...
- 🔐 **A wrong encryption key no longer leaves you staring at a server error with no way in.** Every page steers you to the recovery screen when the key doesn't match your stored secrets, and the sign-in page is deliberately left reachable so an administrator whose session has lapsed can get back in — but that page was itself failing outright while trying to read those secrets, and the recovery screen sends anyone signed out back to it. The result was a loop with no way into the instance at all. Sign-in now renders, says plainly that the key doesn't match, and tells you both ways out: restore the original `ENCRYPTION_KEY` and restart, or sign in with an administrator password and use the recovery screen to discard what can't be read. "Continue with Trakt" is switched off while it's true, since the credential it would authenticate with is one of the values that can't be opened.
...
### 🥚
- Where does it begin, where does it end? Where do you write it all down? How do you reconcile when those answers change? Very carefully.
- A month that ended while nobody was looking in had no one to tell what it held, and was taken at its silence. It keeps what it cannot yet account for now, and asks afterwards, rather than guessing on the spot and calling that the answer.
- Two notices, two different questions. One says what arrived; the other, what has become of it all. They had started finishing each other's sentences.
- It waits to be asked which month, and will reach as far ahead as you care to point — in any order you like. A month is over when the calendar says so, not when you next think to look in on it.
- Turn away from a thing and it stays turned away, on both sides of the glass, whichever month you happened to be standing in at the time. Nothing leaves except by your hand.
- It has learned to ask. Something it has never heard of, or something you walked away from and quietly went back to — an eyebrow raised, rather than an assumption made.
- You finished. Then there was more. It mentions this until you say you've seen it.
- Things wait their turn.
- It asks fewer questions it already knows the answer to, and has stopped writing down what hasn't changed.
- Asked to look back across a span, it hears the span you actually said. And it brings up only what it finds.
- Everything it had, it still has, filed the new way round.
...
- 🔢 **A grayed-out day chip comes back when you show that day again.** Opening the calendar with *Hiding* switched on grayed out the jump-to chips for days whose every show you'd marked not-watching — and switching to *Showing all* left them grayed and unclickable until you reloaded the page. They now become destinations again the moment the day has something on it.
...
- 🪄 **"Apply template"** drops in a ready-made S/A/B/C/D/F set with sensible priorities and colors, so a new board isn't a blank page. On a board that already has tiers it adds only the ones missing.
...
- 🖼️ **Turn a ranking into a poster grid image** you can post anywhere. Choose how many titles (up to 100), how many columns, full size or half, and WebP, JPEG or PNG. Your picture and a title line sit across the top in type sized to read from across a room, each poster carries its rank underneath tinted in its tier's color, and titles underneath are optional — as is a podium row that gives the top three the space they earned. A title whose artwork is missing gets a placeholder tile rather than a hole. Re-exporting something you've already made hands back the same file instantly.
...
- 🖼️ **Upload a profile picture** from your account page. It's centered, cropped to a square and re-encoded on upload, so an odd aspect ratio or leftover photo metadata never makes it into what's stored. Re-uploading replaces it; removing it is one click.
...
- For those who've found it, the way back in no longer turns up a moment after the rest — it's simply there as the page draws, instead of appearing late and nudging its neighbors aside.
...
### Admin interface
- ⚙️ **Settings is organized into tabs** — Server, Trakt, Calendar, Integrations — on a wider panel, instead of one long scroll. It's still one form and one Save, and the "reconnect your Trakt account" prompt sits above the tabs so it can't hide behind the one you're not on.
