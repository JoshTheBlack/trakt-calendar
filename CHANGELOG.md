...
- 🖼️ **Turn a ranking into a poster grid image** you can post anywhere. Choose how many titles (up to 100), how many columns, full size or half, and WebP, JPEG or PNG. Your profile picture and a title line sit across the top, each poster carries its rank underneath tinted in its tier's colour, and titles underneath are optional — as is a podium row that gives the top three the space they earned. A title whose artwork is missing gets a placeholder tile rather than a hole. Re-exporting something you've already made hands back the same file instantly instead of re-rendering it.
- 🔠 **The exported image reads better from across a room.** The title bar across the top is twice the height, so the name and the picture beside it are legible at a glance, and the rank numbers and optional titles under each poster are larger too.
- 📏 **A grid too big for the format you picked is refused before it renders**, with the size it came to, that format's ceiling, and what to change — WebP in particular cannot go beyond 16,383 pixels, which 100 titles in three columns comfortably exceeds. Previously that would have been a long wait ending in an error.
...
- 🚀 **Big boards stay quick.** The pool loads a page at a time as you scroll it rather than all at once, and on a board with a lot already sorted the tiers arrive closed and fill in the first time you open one — so a thousand-title board doesn't cost a thousand posters before you can see anything. If any one part of the page can't be loaded it says so in place with a Retry, rather than taking the board down with it.
...
- 🗄️ **Back up your boards and restore them later**, from the Backup entry in the header menu. One file holds every board, every tier and everything in them, and it restores correctly into a rebuilt database — the file doesn't rely on internal row numbers that only meant something where it was written. Restoring replaces what's there, so it asks you to type the confirmation out. Pictures aren't part of it; they belong to your account rather than to a board, and the dialog says so.
- 👀 **The export dialog previews what you'll get, live**, re-rendering as you change the options, and shows the exact pixel size beside them — so a grid too large for the format you picked is flagged while you're choosing rather than when you press the button. The finished file's size is reported too, with a nudge toward the smaller formats if it's over Discord's 10 MB limit. **Click the preview to zoom in** and move the pointer to look around it — handy for checking the header and your picture, which are a small part of a tall grid.
...
- 🏞️ **Header images now have names, and a place to manage them.** Your account page shows every picture you've saved for the top of an exported grid, with its name editable in place, a Remove for each, and how many of your five slots are used. The export dialog shows the picture you've picked beside its name instead of asking you to choose between "Saved image 2" and "Saved image 3".
