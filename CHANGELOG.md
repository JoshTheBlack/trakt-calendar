...
- 🖱️ **Drag titles from the pool into a tier, between tiers, and up and down within one** — with touch supported, so it works on a phone as well as a mouse. Every move saves itself a moment later, with a quiet "Saved" in the header; if you have the same board open in two tabs, the second one to save is told rather than quietly overwriting the first.
