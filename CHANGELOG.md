...
- 👀 **The export dialog previews what you'll get, live**, re-rendering as you change the options, and shows the exact pixel size beside them — so a grid too large for the format you picked is flagged while you're choosing rather than when you press the button. The finished file's size is reported too, with a nudge toward the smaller formats if it's over Discord's 10 MB limit.
